...
TEST_F(imageTests, constructorFromVectorDataWithParametersThrowsExceptionOnInvalidDimensions) {
    EXPECT_THROW(Image(0, 0, 0, input_vector), std::invalid_argument);
}

TEST_F(imageTests, constructorFromVectorDataWithParametersThowsExceptionOnEmptyVector) {
    vector<Eigen::ArrayXXd> empty_vector;
    EXPECT_THROW(Image(1, 1, 1, empty_vector), std::invalid_argument);
}

TEST_F(imageTests, constructorFromVectorDataWithParametersThrowsExceptionOnInconsistentDimensions) {
    EXPECT_THROW(Image(3, 3, 3, input_vector), std::invalid_argument);
}

TEST_F(imageTests, constructorFromVectorDataWithParametersThrowsExceptionOnInvalidData) {
    EXPECT_THROW(Image(5, 5, 3, invalid_vector), std::invalid_argument);
}

TEST_F(imageTests, constructorFromVectorDataWithParamtersWorksWhenValid) {
    Image image(5, 5, 3, input_vector);
    EXPECT_EQ(image.getChannels(), 3);
...
TEST_F(imageTests, constructorFromEigenArrayWithParametersThrowsExceptionOnInvalidDimensions) {
    EXPECT_THROW(Image(0, 0, 0, input_vector[0]), std::invalid_argument);
}

TEST_F(imageTests, constructorFromEigenArrayWithParametersThrowsExceptionOnInvalidData) {
    EXPECT_THROW(Image(5, 5, 3, invalid_vector[0]), std::invalid_argument);
}

TEST_F(imageTests, constructorFromEigenArrayWithParamtersThrowsExceptionOnInconsistentDimensions) {
    EXPECT_THROW(Image(3, 3, 3, input_vector[0]), std::invalid_argument);
}

TEST_F(imageTests, constructorFromEigenArrayWithParamtersWorksWhenValid) {
    Image image(5, 5, 3, input_vector[0]);
    EXPECT_EQ(image.getChannels(), 3);
    EXPECT_EQ(image.getHeight(), 5);
    EXPECT_EQ(image.getWidth(), 5);
    EXPECT_EQ(image(0, 0, 0), 1);
